package com.jumirandapisousa.nohetica.app.ui.theme

import androidx.compose.ui.graphics.Color

// Cores da identidade visual
val NoeBlue = Color(0xFF1CB1F5)
val NoeGreen = Color(0xFF91D529)
val NoeOrange = Color(0xFFe45900)
val NoeRed = Color(0xFFFF4C4B)
val NoePurple = Color(0xFF594083)
val NeoBlueCard = Color(0xFFC9F3F3)
val NoeTeal = Color(0xFF14806E)

// Cores neutras
val White = Color(0xFFFFFFFF)
val Black = Color(0xFF222222)
val Gray = Color(0xFF666666)
val Background = Color(0xFFFFFFFF)
val MenuBackground = Color(0xFFF8FAFC)
val MenuPressed = Color(0xFFEAF7FD)
val MenuDivider = Color(0xFFE5E7EB)
val MenuText = Color(0xFF263238)
val LegalCardBackground = Color(0xFFF8FAFC)
val BgTermos = Color(0xFFD3EEEB)
